...
| Security signing | Ferocity production | Configured in Netlify production; takes effect on the next deploy | `SECURITY_HMAC_KEY` | Netlify secret env var |
...
| Meta / Facebook | Meta Developers app `Ferocity` under Ferocity Operations | Business Login configuration completed and all four env values staged for the next authorized deploy; administrator testing is available, while third-party customer access still requires Meta review/publishing | `META_APP_ID`, `META_APP_SECRET`, `META_OAUTH_REDIRECT_URI`, `META_BUSINESS_LOGIN_CONFIG_ID` | Netlify env vars / password manager |
| Premium video rendering | `ferocityflow1@gmail.com`, Google project `Ferocity` | Production is currently configured for OpenAI Sora 2 with platform/workspace caps and profitable per-second pricing. Google AI Studio billing exists, but the Google Veo key/model and one controlled live Veo render are not yet verified. | `VIDEO_PROVIDER`, `VIDEO_API_KEY`, `VIDEO_MODEL`, `VIDEO_RENDERING_ENABLED`, `VIDEO_MONTHLY_BUDGET_CENTS`, `VIDEO_WORKSPACE_MONTHLY_BUDGET_CENTS`, pricing vars | Netlify env vars / password manager |
| TikTok | Likely `ferocityflow1@gmail.com`; username `ferocity78`; Business Center `7666569313026654224`, ad account `7666569270685155345` | OAuth app keys are configured in Netlify production. Ads Manager payment/agreement readiness is still incomplete; promotions previously showed `$0.00`. | `TIKTOK_CLIENT_KEY`, `TIKTOK_CLIENT_SECRET`, `TIKTOK_OAUTH_REDIRECT_URI` | Netlify env vars / password manager; resolve agreement/billing before spend |
