...
export async function createReceiptExpenseAction(formData: FormData) {
  ...
  await queryPostgres(
    ...
    values ($1,$2,$3,$4,current_date,$5,$6,$7,'job',$8,$9,'needs_review',$10,$11,$12,$13::jsonb)
    `,
    [
      tenantId,
      parsed.data.workerId ?? null,
      ...
      parsed.data.notes ?? "Receipt submitted for owner review.",
      parsed.data.reimbursementStatus,
      parsed.data.reimbursementDueDate || null,
      parsed.data.notes ?? null,
      JSON.stringify({ source: "job_tracker", receiptFlow: true, ownerReminder: parsed.data.reimbursementStatus === "submitted" })
    ]
  );
  await refresh();
