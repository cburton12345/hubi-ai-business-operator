...
export async function runRevenueGrowthScanAction() {
  ...
  await queryPostgres(
    ...
    insert into public.revenue_lead_scores (
      ...
      estimated_value_cents, recommended_next_action, qualification_reason, disqualification_reason, scoring_inputs_json, last_scored_at, updated_at
    )
    select
      l.tenant_id,
      l.brand_id,
      ...
      case
        ...
        when l.qualification_status = 'qualified' or l.priority = 'high' or l.lead_type in ('appointment','quote','case_intake') then 'qualified'
      ...
      jsonb_build_object(
        ...
        'priority', l.priority,
        'hasPhone', l.phone is not null,
        'hasEmail', l.email is not null,
        'consentToContact', l.consent_to_contact,
    ...
    on conflict (tenant_id, lead_id) do update set
      ...
      scoring_inputs_json = excluded.scoring_inputs_json,
      last_scored_at = now(),
      updated_at = now()
    `,
    [workspaceId]
